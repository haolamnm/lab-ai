...
2.  Pick a frontier. UCS orders by ``g`` (cost so far), so a
    :class:`~route_lab.shared.heap.Heap` keyed on it.
3.  Loop: take the next state from :func:`pop_fresh`, which drops the stale
    entries an improvement left behind; :func:`record_expansion` and test the goal.
...
and pass that estimate to ``record_expansion``. BFS/DFS swap the ``Heap`` for a
``Queue``/``Stack`` and count cost in hops. That is the entire difference
...
def uniform_cost_search(problem: SearchProblem) -> SearchLegResult:
    ...
    while (current := pop_fresh(frontier, memory)) is not None:
        ...
        for edge, successor in next_states(memory, current):
            ...
            if successor in memory.cost and candidate_cost >= memory.cost[successor]:
                continue
