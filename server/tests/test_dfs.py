...
def test_dfs_takes_the_deep_branch_where_bfs_takes_the_shallow_one() -> None:
    ...
    deep = depth_first_search(graph_problem(_FORK_NODES, _FORK_EDGES, "S", "G"))
    shallow = breadth_first_search(graph_problem(_FORK_NODES, _FORK_EDGES, "S", "G"))
...
def test_dfs_is_blind() -> None:
    leg = depth_first_search(diamond_problem())
    assert leg.trace
    assert all(step.h is None for step in leg.trace)


def test_dfs_trace_g_counts_depth() -> None:
    leg = depth_first_search(graph_problem(_FORK_NODES, _FORK_EDGES, "S", "G"))
...
def test_dfs_reports_no_route_when_the_goal_is_unreachable() -> None:
    leg = depth_first_search(
        graph_problem(["S", "M", "G"], [{"from": "S", "to": "M", "km": 1.0}], "S", "G")
    )
...
def test_dfs_follows_one_way_streets() -> None:
    leg = depth_first_search(diamond_problem("D", "A"))
    assert leg.found is False
